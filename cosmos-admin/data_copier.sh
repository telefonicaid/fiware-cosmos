...
# Show the usage
if [ $# -ne 4 ]; then
	echo "Usage: ./data_copier.sh <usernames_file> <src_cluster_URL> <dst_cluster_URL> <max_bytes_transfer>"
	exit 1;
fi
...
tmpDir=$(mktemp -d /tmp/data_mover.XXXX)
...
migrate() {
	...
	while read -r entry; do
		...
		length=$(curl -X GET "$srcClusterURL/webhdfs/v1$path?op=liststatus&user.name=hdfs" 2> /dev/null | \
			python -m json.tool | grep length | awk '{ print $2 }' | sed 's/"//g' | sed 's/,//g' | sed $index'q;d')
		...
		if test $length -eq 0; then
			...
			curl -X PUT "$dstClusterURL/webhdfs/v1$path/$entry?op=mkdirs&user.name=hdfs" 2> /dev/null > /dev/null
			curl -X PUT "$dstClusterURL/webhdfs/v1$path/$entry?op=setowner&owner=$username&group=$username&user.name=hdfs" \
				2> /dev/null > /dev/null
			...
			curl -X GET "$srcClusterURL/webhdfs/v1$path/$entry?op=open&user.name=hdfs" > $tmpFile 2> /dev/null
			...
			for i in `seq 0 $numChunks`; do
				...
				if test $i -eq 0; then
					...
					dd if=$tmpFile ibs=1 skip=0 count=$bytes 2> /dev/null | \
						curl -L --data-binary @- -X PUT "$dstClusterURL/webhdfs/v1$path/$entry?op=create&user.name=hdfs" \
						2> /dev/null > /dev/null
				else
					# This is other chunk, thus append the data
					dd if=$tmpFile ibs=1 skip=$offset count=$bytes 2> /dev/null | \
						curl -L --data-binary @- -X POST "$dstClusterURL/webhdfs/v1$path/$entry?op=append&user.name=hdfs" \
						2> /dev/null > /dev/null
				fi
			done
		...
		index=$((index+1))
	done < <(curl -X GET "$srcClusterURL/webhdfs/v1$path?op=liststatus&user.name=hdfs" 2> /dev/null | \
...
while read -r username; do
	...
	curl -X PUT "$dstClusterURL/webhdfs/v1/user/$username?op=mkdirs&user.name=hdfs" 2> /dev/null > /dev/null
	curl -X PUT "$dstClusterURL/webhdfs/v1/user/$username?op=setowner&owner=$username&group=$username&user.name=hdfs" \
		2> /dev/null > /dev/null
